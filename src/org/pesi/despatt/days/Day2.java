...
public class Day2 implements Day {
	...
	String numString = "1919\t2959\t82\t507\t3219\t239\t3494\t1440\t3107\t259\t3544\t683\t207\t562\t276\t2963\n"
...
//	String numString = "8\t7\t3\t4\n2\t9\t6\t5\n";
	String[] rows = numString.split("\\n");
	ArrayList<Integer> numbers;
	int difference = 0;

	@Override
	public void partOne() {
		//put string into a Collection, sort and find diff btwn largest & smallest
		for (String row : rows){
			numbers = new ArrayList<>();
			String[] numberTokens = row.split("\\s");
			for (String number : numberTokens){
				numbers.add(Integer.valueOf(number));
			}
				Collections.sort(numbers);
				difference += numbers.get(numbers.size() - 1) - numbers.get(0);
		}
	...
	public void partTwo() {
		Integer result = 0;
		for (String row : rows) { //2 rows of numbers
			//
			ArrayList<Integer> numbers = new ArrayList<>();
			String[] numbersInRow = row.split("\\s");
